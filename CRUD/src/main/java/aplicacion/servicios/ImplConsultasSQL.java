...
/**
 * Implementacion de la interfaz consultas sql
 */
public class ImplConsultasSQL implements InterfazConsultasSQL {
	...
	public void updateLibro(Connection conexion) {
		...
		try {
			...
			declaracion = conexion.prepareStatement(
					"update gbp_almacen.gbp_alm_cat_libros set titulo=?, autor=?, isbn=?, edicion=? where isbn=?");
	...
	/**
	 * Método que preguntara lo que se pasa por parametro Si responde que si
	 * devuelve true Si responde que no devuelve false
	 * 
	 * @param txt
	 * @return
	 */
