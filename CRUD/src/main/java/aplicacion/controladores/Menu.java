...
import aplicacion.servicios.ImplConsultasSQL;
import aplicacion.servicios.ImplMenu;
import aplicacion.servicios.InterfazConexionSQL;
import aplicacion.servicios.InterfazConsultasSQL;
...
public class Menu {
	...
	public static void main(String[] args) {
		...
		InterfazConsultasSQL icsq = new ImplConsultasSQL();
		...
		do {
			...
			if (opcion <= 4 && opcion >= 1) {
				try {
					...
					System.err.println(
							"** [ERROR-main] No se ha podido conectar a la base de datos. " + e.getMessage() + " **");
				}
			}
					...
					try {
						...
						for (LibroDTO aux : listaLibros) {
							System.out.printf("Id = %s , Titulo = %s, Autor = %s, Isbn = %s, Edicion = %s \n",
									aux.getIdLibro(), aux.getTitulo(), aux.getAutor(), aux.getIsbn(), aux.getEdicion());
						}
					} catch (Exception e) {
